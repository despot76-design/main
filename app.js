...
{id:'g',name:'샘플 노트북 G',brand:'SAMPLE',cpu:'CPU Eta 5',cpuYear:'2024',cpuGen:'2세대 전',cpuSingle:78,cpuMulti:80,cpuScore:79,gpu:'내장 GPU G',gpuScore:72,npu:'미지원',ram:'16GB',ramType:'DDR5',ramExpand:'가능',ssd:'512GB',ssdExpand:'가능',display:'15.6 IPS',resolution:'1920×1080',brightness:'300 nit',refresh:'60Hz',color:'sRGB 62%',weight:1.68,battery:54,ports:'USB-C 1 / USB-A 2 / HDMI',wifi:'Wi-Fi 6',os:'Windows 11',wow:649000,normal:699000,avg30:735000,low30:639000,value:92,performance:78,portability:74,displayScore:70,verdict:'예산형 추천',use:'문서 · 인터넷 · 학습'},
{id:'h',name:'샘플 노트북 H',brand:'SAMPLE',cpu:'CPU Theta HX 9',cpuYear:'2026',cpuGen:'최신',cpuSingle:96,cpuMulti:99,cpuScore:98,gpu:'외장 GPU H',gpuScore:98,npu:'지원',ram:'32GB',ramType:'DDR5',ramExpand:'가능',ssd:'2TB',ssdExpand:'가능',display:'16.0 OLED',resolution:'3200×2000',brightness:'600 nit',refresh:'165Hz',color:'DCI-P3 100%',weight:2.28,battery:90,ports:'USB-C 2 / USB-A 2 / HDMI / LAN / SD',wifi:'Wi-Fi 7',os:'Windows 11',wow:2199000,normal:2399000,avg30:2460000,low30:2150000,value:93,performance:99,portability:48,displayScore:99,verdict:'고성능 특가',use:'게임 · 3D · 영상편집'}
];

let wow=true;
let selected=new Set(products.map(p=>p.id));
...
function levelAllowed(row){if(detailLevel==='expert')return true;if(detailLevel==='basic')return row.level!=='expert';return row.level==='simple';}
function uniqueValues(row,list){return new Set(list.map(p=>String(row.value(p)))).size;}
function bestValue(row,list){if(!row.type||!list.length)return null;const vals=list.map(p=>row.value(p)).filter(v=>typeof v==='number'&&!Number.isNaN(v));if(!vals.length)return null;return row.type==='max'?Math.max(...vals):Math.min(...vals);}
function drawRanking(){const sorted=[...products].sort((a,b)=>b.value-a.value);rankingCards.innerHTML=sorted.map((p,i)=>`<article class="rank-card"><div class="rank">#${i+1} · 가성비 ${p.value}점</div><h3>${p.name}</h3><div class="cpu">${p.cpu} · ${p.cpuGen}</div><div class="price"><strong>${fmt(wow?p.wow:p.normal)}</strong><span class="score">성능 ${p.performance}</span></div><button class="rank-add ${selected.has(p.id)?'selected':''}" data-id="${p.id}">${selected.has(p.id)?'✓ 비교함에 담김':'＋ 비교함 담기'}</button></article>`).join('');const top=sorted[0];heroWinner.textContent=top.name;heroPrice.textContent=fmt(wow?top.wow:top.normal);heroScore.textContent=top.value+'점';}
function drawPicker(){productPicker.innerHTML=products.map(p=>`<button class="pick-chip ${selected.has(p.id)?'active':''}" data-id="${p.id}">${selected.has(p.id)?'✓':'＋'} ${p.name}</button>`).join('');compareCount.textContent=selected.size;}
function drawTable(){const list=products.filter(p=>selected.has(p.id));if(!list.length){compareTable.innerHTML='';emptyCompare.style.display='block';return;}emptyCompare.style.display='none';const diff=document.getElementById('diffOnly').checked;let rows=rowDefs.filter(levelAllowed).filter(r=>!diff||uniqueValues(r,list)>1);let html='<thead><tr><th>비교 항목</th>'+list.map(p=>`<th><div class="table-product"><b>${p.name}</b><small>${p.cpu}</small><button class="remove-btn" data-id="${p.id}">비교에서 빼기</button></div></th>`).join('')+'</tr></thead><tbody>';let lastGroup='';for(const row of rows){if(row.group!==lastGroup){html+=`<tr class="group-row"><td colspan="${list.length+1}">${row.group}</td></tr>`;lastGroup=row.group;}const best=bestValue(row,list);html+=`<tr><td>${row.label}</td>`+list.map(p=>{const raw=row.value(p);const cls=best!==null&&raw===best?'best':'';return `<td class="${cls}">${row.format(raw)}${cls?' <span class="best-tag">BEST</span>':''}</td>`}).join('')+'</tr>';}compareTable.innerHTML=html+'</tbody>';}
function redraw(){drawRanking();drawPicker();drawTable();}
function toggle(id){if(selected.has(id))selected.delete(id);else if(selected.size<8)selected.add(id);else alert('한 번에 최대 8대까지 비교할 수 있습니다.');redraw();}

document.addEventListener('click',e=>{const chip=e.target.closest('.pick-chip,.rank-add');if(chip){toggle(chip.dataset.id);return;}const rem=e.target.closest('.remove-btn');if(rem){selected.delete(rem.dataset.id);redraw();return;}const detail=e.target.closest('.detail-btn');if(detail){document.querySelectorAll('.detail-btn').forEach(b=>b.classList.remove('active'));detail.classList.add('active');detailLevel=detail.dataset.level;drawTable();}});
